...
public class IntroPane extends BorderPane {
    ...
    public IntroPane(State state) {
        ...
        for (int i = 0; i < cardImagesViews.length; i++) {
            ...
            if (i == 2) cardTranslate.setToX(state.getScreenWidth() + cardImagesViews[i - 1].getFitWidth() * 10);
            else cardTranslate.setToX(state.getScreenWidth() + cardImagesViews[i].getFitWidth() * 2);
